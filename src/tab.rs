...
pub enum SyncStatus {
    ...
    /// placeholder). Rendered dimmed, like OneDrive/Dropbox online-only files.
    OnlineOnly,
}
...
impl SyncStatus {
    ...
    /// Icon opacity for this status — online-only files are dimmed to signal
    /// they are not on this device; everything else renders fully opaque.
    fn icon_opacity(self) -> f32 {
        match self {
            SyncStatus::OnlineOnly => 0.4,
            _ => 1.0,
        }
    }
...
fn item_icon<'a>(
    ...
    let base = widget::icon::icon(handle)
        ...
        .size(size)
        .opacity(status.map_or(1.0, SyncStatus::icon_opacity));
    match status {
        Some(status) => {
